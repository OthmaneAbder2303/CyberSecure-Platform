...
// Endpoint for generating certificates
...
// Endpoint for generating signatures
app.post('/generate-signature', (req, res) => {
    const { dataToSign, privateKeyPem } = req.body;

    if (!dataToSign || !privateKeyPem) {
        return res.status(400).json({ message: 'Data to sign and private key are required' });
    }
...
// Endpoint for signing documents
app.post('/sign-document', upload.single('document'), (req, res) => {
    const { privateKeyPem } = req.body;
    const documentPath = req.file.path;

    if (!privateKeyPem || !documentPath) {
        return res.status(400).json({ message: 'Private key and document are required' });
    }
...
// Endpoint for generating certificates for email encryption
app.post('/generate-email-cert', upload.fields([{ name: 'idDocument' }, { name: 'proofOfAddress' }, { name: 'authorizationLetter' }]), (req, res) => {
    const fullName = req.body.fullName;
    const email = req.body.email;
    const phone = req.body.phone;
    const organization = req.body.organization;
    ...
    // Save certificate and private key to files
    const certPath = path.join(__dirname, `${email}-email-cert.pem`);
    const keyPath = path.join(__dirname, `${email}-email-key.pem`);
    
    fs.writeFileSync(certPath, pem.certificate);
    fs.writeFileSync(keyPath, pem.privateKey);

    // Save timestamp
    const timestampPath = path.join(__dirname, `${email}-email-timestamp.txt`);
    fs.writeFileSync(timestampPath, timestamp);

    res.json({
        message: 'Email certificate generated successfully!',
        certDownloadLink: `http://localhost:3000/download/${email}-email-cert.pem`,
        keyDownloadLink: `http://localhost:3000/download/${email}-email-key.pem`,
        timestampDownloadLink: `http://localhost:3000/download/${email}-email-timestamp.txt`
    });
});

// Endpoint for sending encrypted emails
app.post('/send-email', (req, res) => {
    ...
    if (!senderEmail || !senderPassword || !receiverEmail || !emailSubject || !emailContent) {
        return res.status(400).json({ message: 'All fields are required' });
    }

    // Load the sender's certificate and private key
    const certPath = path.join(__dirname, `${senderEmail}-email-cert.pem`);
    const keyPath = path.join(__dirname, `${senderEmail}-email-key.pem`);

    if (!fs.existsSync(certPath) || !fs.existsSync(keyPath)) {
        return res.status(404).json({ message: 'Certificate or key not found' });
    }
    ...
    const transporter = nodemailer.createTransport({
        ...
        auth: {
            user: senderEmail,
            pass: "zmho iktj lphh xfkz "  // Use the provided password here
        }
    });
